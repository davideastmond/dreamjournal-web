...
function RegistrationModal (props:IRegistrationModalProps) {
  ...
  const [registrationPassword1, setRegistrationPassword1] = useState<string>("");
  const [registrationPassword2, setRegistrationPassword2] = useState<string>("");

  const [emailErrorState, setEmailErrorState] = useState<boolean>(false);
  const [firstNameErrorState, setFirstNameErrorState] = useState<boolean>(false);
  const [lastNameErrorState, setLastNameErrorState] = useState<boolean>(false);
  const [password1ErrorState, setPassword1ErrorState] = useState<boolean>(false);
  const [password2ErrorState, setPassword2ErrorState] = useState<boolean>(false);
  ...
  <div>
    <Dialog open={props.open} onClose={props.onDismiss}>
      ...
      <DialogActions>
        ...
        <Button color="success" onClick={handleSubmitRegistrationRequest}>Register</Button>
      </DialogActions>
